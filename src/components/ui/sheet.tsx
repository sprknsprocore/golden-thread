...
function SheetContent({
  ...
  return (
    <SheetPortal>
      ...
      >
        {children}
        {/* Procore tearsheet: floating close button on the scrim side */}
        {showCloseButton && (
          <SheetPrimitive.Close
            className={cn(
              "absolute z-[60] flex h-9 w-9 items-center justify-center rounded-full bg-white/90 shadow-md ring-1 ring-black/5 transition-all hover:bg-white hover:shadow-lg focus:ring-2 focus:ring-offset-2 focus:outline-hidden",
              side === "right" && "-left-12 top-4",
              side === "left" && "-right-12 top-4",
              side === "top" && "left-1/2 -bottom-12 -translate-x-1/2",
